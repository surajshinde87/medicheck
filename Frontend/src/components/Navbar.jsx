...
const logout = ()=>{
  ...
  return (
    <div className='flex items-center justify-between text-sm mb-5 py-4 border-b border-b-gray-400 '>
      <img onClick={()=>navigate('/')} src={assets.logo} className='w-40 lg:w-[300px]  cursor-pointer' alt="logo" />
      <ul className='hidden md:flex items-start gap-5 font-semibold text-xl'>
        <NavLink to={'/'}>
      ...
      <div className='flex items-center gap-4'>
        {
            ...
            ? <div className='flex items-center gap-2 cursor-pointer group relative'>
                <img className='w-10 h-10 block mx-auto object-center rounded-full' src={userData.image} alt="profile-picture" />
        ...
        <div className={`${showMenu ? 'fixed w-full' : 'h-0 w-0'} md:hidden right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all`}>
          ...
          <ul className='flex flex-col items-center gap-5 mt-5 font-medium px-5 text-lg'>
